...
module Saw
  module Logger
    ...
    DEFAULT_OPTS = {:trace => false, :console_level => DEFAULT_SCREEN_LEVEL, :file_level => DEFAULT_FILE_LEVEL }
    ...
    class SawFormatter <  Log4r::Formatter
      ...
      def initialize(hash={})
        ...
        @color = hash[:color] or hash['color'] or true
      end

      def format(event)
        color = Saw::Logger::LOG_COLORS[event.level - 1]
